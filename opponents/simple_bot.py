...
Usage: python3 opponents/simple_bot.py [--wallet N]
  N = 1-5 (default 1), selects OPPONENT_N_PRIVATE_KEY from .env
"""
import argparse
import json
import os
...
GAME_TYPE_RPS = 0
MOVE_ROCK, MOVE_PAPER, MOVE_SCISSORS = 1, 2, 3
...
class Bot:
    """Simple opponent bot with its own wallet and web3 connection."""

    def __init__(self, wallet_num: int):
        ...
        # Track active games we're playing
        self.active_games = {}  # game_id -> {saved_moves, saved_salts}
    ...
    def ensure_registered(self):
        ...
        try:
            ...
            if info[5]:  # exists field
                print(f"  Already registered. ELO: {self.registry.functions.elo(self.address, GAME_TYPE_RPS).call()}")
                return
        except Exception:
            pass
        ...
        max_wager = int(1.0 * 10**18)
        print(f"  Registering for RPS (wager 0.001 - 1.0 MON)...")
        receipt = self.send_tx(
            self.registry.functions.register([GAME_TYPE_RPS], min_wager, max_wager)
        )
    ...
    def scan_for_challenges(self):
        ...
        for mid in range(next_id):
            ...
            try:
                ...
                # m = (player1, player2, wager, gameContract, status, createdAt)
                if m[1].lower() == self.address.lower() and m[4] == MATCH_CREATED:
                    new_challenges.append((mid, m))
                # Mark as known regardless of whether it's for us
    ...
    def find_game_for_match(self, match_id: int) -> int | None:
        """Find a game ID linked to an escrow match, if one exists."""
    ...
    def play_game_tick(self, game_id: int):
        ...
        # game = (escrowMatchId, player1, player2, totalRounds, currentRound,
        ...
        if game_id not in self.active_games:
            self.active_games[game_id] = {"moves": {}, "salts": {}, "acted": set()}
        ...
        # Use action key to avoid re-attempting failed transactions
        ...
        if phase == PHASE_COMMIT and action_key not in gdata["acted"]:
            ...
            if not my_committed:
                move = random.choice([MOVE_ROCK, MOVE_PAPER, MOVE_SCISSORS])
                ...
                print(f"    Round {current_round + 1}/{game[3]}: Committing {MOVE_NAMES[move]}...")
        ...
        # Reveal phase
            ...
            if not my_revealed:
                ...
                if move is None or salt is None:
                    ...
                    return False
                print(f"    Round {current_round + 1}/{game[3]}: Revealing {MOVE_NAMES[move]}...")
    ...
    def _print_result(self, game_id: int, game: tuple):
        ...
        if my_score > opp_score:
            ...
            result = "DRAW"
        print(f"    Game {game_id} complete! Score: {my_score}-{opp_score} ({result})")
        # Clean up
    ...
    def run(self):
        ...
        print(f"\n{'='*60}")
        print(f"Simple Bot #{self.wallet_num}")
        ...
        # Track active game IDs to play each tick
        playing_games = set()

        print("Polling for challenges... (Ctrl+C to stop)\n")
        while True:
            try:
                    ...
                    try:
                        ...
                        if (m[1].lower() == self.address.lower()
                            ...
                            if gid is not None and gid not in playing_games:
                                # Check game isn't already settled
                                g = self.rps_game.functions.getGame(gid).call()
...
def main():
    parser = argparse.ArgumentParser(description="Simple opponent bot for Gaming Arena")
    parser.add_argument("--wallet", type=int, default=1, choices=range(1, 6),
                        help="Opponent wallet number (1-5, default: 1)")
