...
program
    ...
    .action(wrapCommand(async (opts: any) => {
        const addr = opts.address || getAddressFromKey();
        if (!addr) { fail("Provide --address or set PRIVATE_KEY", "NO_ADDRESS"); return; }
...
program
    ...
    .action(wrapCommand(async (gameTypes: string, opts: any) => {
...
program
    ...
    .argument("<game_type>", "Game type: rps, poker, or auction")
    .action(wrapCommand(async (address: string, wager: string, gameType: string) => {
...
program
    ...
    .action(wrapCommand(async (gameId: string, action: string, amount?: string) => {
...
program
    ...
    .action(wrapCommand(async (marketId: string, side: string, amount: string) => {
