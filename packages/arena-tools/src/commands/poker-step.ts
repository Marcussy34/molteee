...
import { pokerGameAbi, escrowAbi } from "../contracts.js";
...
export async function pokerStepCommand(gameId: string, decision: string, opts: { amount?: string }) {
  ...
  const game: any = await retry(
    () => client.readContract({ address: addr, abi: pokerGameAbi, functionName: "getGame", args: [gid] }),
    "getGame"
  );
  ...
  if (phase === PHASE_COMMIT) {
    ...
    if (!myCommitted) {
      ...
      const { hash: txHash } = await retry(
        () => sendTx({ to: addr, data: encodeFunctionData({ abi: pokerGameAbi, functionName: "commitHand", args: [gid, hash] }) }),
        "commit"
      );
    ...
    while (true) {
      ...
      if (Number(g.phase) !== PHASE_COMMIT) {
        const isMyTurn = (g.currentTurn as string).toLowerCase() === myAddress.toLowerCase();
        ok({
          gameId: Number(gid),
          phase: PHASE_NAMES[Number(g.phase)],
          isYourTurn: isMyTurn,
          pot: formatEther(g.pot as bigint),
          currentBet: formatEther(g.currentBet as bigint),
          gameComplete: false,
  ...
  if (phase === POKER_BETTING1 || phase === POKER_BETTING2) {
    const isMyTurn = (game.currentTurn as string).toLowerCase() === myAddress.toLowerCase();
    ...
    if (isMyTurn) {
      ...
      if (decision.toLowerCase() === "bet" || decision.toLowerCase() === "raise") {
        ...
        value = game.currentBet as bigint;
      }
    ...
    while (true) {
      ...
      const nowMyTurn = (g.currentTurn as string).toLowerCase() === myAddress.toLowerCase();
