...
export async function auctionRoundCommand(gameId: string, bid: string) {
  ...
  const addr = CONTRACTS.AuctionGame as `0x${string}`;

  const bidWei = parseEther(bid);

  // Read game state
  const game: any = await retry(
    () => client.readContract({ address: addr, abi: auctionGameAbi, functionName: "getGame", args: [gid] }),
    "getGame"
  );
  ...
  if (phase === PHASE_COMMIT && !myCommitted) {
    ...
    const { hash: txHash } = await retry(
      () => sendTx({ to: addr, data: encodeFunctionData({ abi: auctionGameAbi, functionName: "commitBid", args: [gid, hash] }) }),
      "commit"
    );
  ...
  while (true) {
    ...
    if (g.settled) {
      ...
      const myBidMON = formatEther(myBid as bigint);
      const oppBidMON = formatEther(oppBid as bigint);
      const result = (myBid as bigint) > (oppBid as bigint) ? "win" : (myBid as bigint) < (oppBid as bigint) ? "loss" : "draw";

      ok({
        gameId: Number(gid),
        yourBid: myBidMON,
        opponentBid: oppBidMON,
        prize: formatEther(g.prize as bigint),
        gameComplete: true,
        gameResult: result,
