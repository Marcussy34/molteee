...
// ─── Main command ─────────────────────────────────────────────────────────
export async function auctionRoundCommand(gameId: string, bid: string) {
    ...
    // Read game state
    const game = await retry(() => client.readContract({ address: addr, abi: auctionGameAbi, functionName: "getGame", args: [gid] }), "getGame") as any;
    ...
    while (true) {
        ...
        const g = await retry(() => client.readContract({ address: addr, abi: auctionGameAbi, functionName: "getGame", args: [gid] }), "poll-commit") as any;
        if (g.settled) {
    ...
    const game2 = await retry(() => client.readContract({ address: addr, abi: auctionGameAbi, functionName: "getGame", args: [gid] }), "getGame-reveal") as any;
    ...
    while (true) {
        ...
        const g = await retry(() => client.readContract({ address: addr, abi: auctionGameAbi, functionName: "getGame", args: [gid] }), "poll-reveal") as any;
        if (g.settled) {
