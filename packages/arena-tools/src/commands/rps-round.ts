...
function roundWinner(p1Move: number, p2Move: number): string {
    if (p1Move === p2Move) return "draw";
    // Rock(1) beats Scissors(3), Scissors(3) beats Paper(2), Paper(2) beats Rock(1)
    if ((p1Move === 1 && p2Move === 3) || (p1Move === 3 && p2Move === 2) || (p1Move === 2 && p2Move === 1))
        return "p1";
    return "p2";
}
...
export async function rpsRoundCommand(gameId: string, move: string) {
    ...
    // Read game state
    const game = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "getGame") as any;
    ...
    // ── Step 1: Commit ──────────────────────────────────────────────────────
    const roundData = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound") as any;
    ...
    while (true) {
        ...
        const g = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "poll-commit") as any;
        if (g.settled) {
    ...
    const roundData2 = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound-reveal") as any;
    ...
    if (!myRevealed) {
        ...
        for (let revealAttempt = 0; revealAttempt < 3; revealAttempt++) {
            try {
                ...
                const gCheck = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "getGame-after-revert") as any;
    ...
    while (true) {
        ...
        const g = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "poll-reveal") as any;

        // Game settled — final result
        if (g.settled) {
            // Read final round data for move info
            const finalRound = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound-final") as any;
        ...
        if (Number(g.currentRound) > round) {
            const finalRound = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound-result") as any;
            const p1Move = Number(finalRound.p1Move);
            const p2Move = Number(finalRound.p2Move);
            const myMove = isPlayer1 ? p1Move : p2Move;
            const oppMove = isPlayer1 ? p2Move : p1Move;
