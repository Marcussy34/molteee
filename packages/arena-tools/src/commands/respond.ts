...
function gameTypeFromAddress(addr: string): string | null {
    const lower = addr.toLowerCase();
    for (const [name, contractAddr] of Object.entries(GAME_CONTRACTS)) {
        if (contractAddr.toLowerCase() === lower) return name;
    }
    return null;
}
...
export async function respondCommand(matchId: string, opts: any) {
    ...
    const match = (await retry(
        () => client.readContract({ address: CONTRACTS.Escrow, abi: escrowAbi, functionName: "getMatch", args: [id] }),
        "getMatch"
    )) as any;
    ...
    let winner: string = "0x0000000000000000000000000000000000000000";
    try {
        winner = (await retry(
            () => client.readContract({ address: CONTRACTS.Escrow, abi: escrowAbi, functionName: "winners", args: [id] }),
            "getWinner"
        )) as any as string;
    }
    catch { /* not set yet */ }
...
async function createGame(client: any, gameType: string, matchId: bigint, rounds: number, contractAddr: `0x${string}`): Promise<bigint> {
...
function loadStoredSalt(key: string): any {
    // Memory first
