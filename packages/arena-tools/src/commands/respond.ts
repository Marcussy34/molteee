...
const RPS_MOVES = [1, 2, 3] as const;
const RPS_MOVE_NAMES: Record<number, string> = { 1: "Rock", 2: "Paper", 3: "Scissors" };
...
function gameTypeFromAddress(addr: string): GameTypeName | null {
  const lower = addr.toLowerCase();
  for (const [name, contractAddr] of Object.entries(GAME_CONTRACTS)) {
    if (contractAddr.toLowerCase() === lower) return name as GameTypeName;
  }
  return null;
}

function gameContractAddr(gameType: GameTypeName): `0x${string}` {
  return ({ rps: CONTRACTS.RPSGame, poker: CONTRACTS.PokerGame, auction: CONTRACTS.AuctionGame })[gameType] as `0x${string}`;
}
...
export async function respondCommand(
  matchId: string,
  opts: { rounds?: string; timeout?: string }
) {
  ...
  const match = await retry(
    () => client.readContract({ address: CONTRACTS.Escrow as `0x${string}`, abi: escrowAbi, functionName: "getMatch", args: [id] }),
    "getMatch"
  );
  ...
  const gameType = gameTypeFromAddress(match.gameContract as string);
  if (!gameType) {
    fail(`Unknown game contract ${match.gameContract}.`, "UNKNOWN_GAME");
    return;
  }

  const wagerWei = match.wager as bigint;
  const isPlayer1 = (match.player1 as string).toLowerCase() === myAddress.toLowerCase();

  // Accept if needed, or wait for acceptance if we're the challenger
  if (status === MATCH_CREATED) {
    if ((match.player2 as string).toLowerCase() === myAddress.toLowerCase()) {
  ...
  try {
    winner = (await retry(
      () => client.readContract({ address: CONTRACTS.Escrow as `0x${string}`, abi: escrowAbi, functionName: "winners", args: [id] }),
      "getWinner"
    )) as string;
  } catch { /* not set yet */ }
...
async function rpsLoop(client: any, gameId: bigint, addr: `0x${string}`, isPlayer1: boolean, startTime: number, timeoutMs: number) {
  while (true) {
    checkTimeout(startTime, timeoutMs);

    // 1. Read game state
    const game: any = await retry(
      () => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gameId] }),
      "rps-getGame"
    );
    ...
    const roundData: any = await retry(
      () => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gameId, BigInt(round)] }),
      "rps-getRound"
    );
...
async function pokerLoop(client: any, gameId: bigint, addr: `0x${string}`, isPlayer1: boolean, wagerWei: bigint, startTime: number, timeoutMs: number) {
  while (true) {
    ...
    const isMyTurn = (game.currentTurn as string).toLowerCase() === getAddress().toLowerCase();
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function auctionLoop(client: any, gameId: bigint, addr: `0x${string}`, isPlayer1: boolean, wagerWei: bigint, startTime: number, timeoutMs: number) {
  while (true) {
    checkTimeout(startTime, timeoutMs);

    const game: any = await retry(
      () => client.readContract({ address: addr, abi: auctionGameAbi, functionName: "getGame", args: [gameId] }),
      "auction-getGame"
    );
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function findGame(client: any, contractAddr: `0x${string}`, gameType: GameTypeName, matchId: bigint): Promise<bigint | null> {
  const abi = gameType === "rps" ? rpsGameAbi : gameType === "poker" ? pokerGameAbi : auctionGameAbi;

  let nextId: bigint;
  try {
    nextId = (await retry(
      () => client.readContract({ address: contractAddr, abi: nextGameIdAbi, functionName: "nextGameId" }),
      "nextGameId"
    )) as bigint;
  } catch {
    return null;
  ...
  for (let i = start; i >= end; i--) {
    try {
      ...
      if (BigInt(game.escrowMatchId) === matchId) return BigInt(i);
    } catch { continue; }
  }
  return null;
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function createGame(client: any, gameType: GameTypeName, matchId: bigint, rounds: number, contractAddr: `0x${string}`): Promise<bigint> {
  // Read nextGameId before creating — that will be our game's ID
  const beforeId = (await retry(
    () => client.readContract({ address: contractAddr, abi: nextGameIdAbi, functionName: "nextGameId" }),
    "nextGameId-pre"
  )) as bigint;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function storeSalt(key: string, value: any): void {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function loadStoredSalt(key: string): any | null {
  // Memory first
