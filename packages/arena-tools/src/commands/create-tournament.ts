...
export async function createTournamentCommand(
  format: string,
  maxPlayers: string,
  opts: { entryFee: string; baseWager: string }
) {
  // Validate format
  const formatCode = FORMAT_MAP[format.toLowerCase()];
  if (formatCode === undefined) {
    fail(
      `Invalid format: ${format}. Must be round-robin or double-elim.`,
      "INVALID_FORMAT"
    );
    return;
  }
  ...
  if (players !== 4 && players !== 8) {
    fail(
      `Invalid max_players: ${maxPlayers}. Must be 4 or 8.`,
      "INVALID_PLAYERS"
    );
    return;
  }
  ...
  const { hash } = await sendTx({
    to: CONTRACTS.TournamentV2 as `0x${string}`,
    data,
  });
