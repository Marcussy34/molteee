...
async function executeBettingAction(game, gid, addr, myAddress, betAction, ifBetAction, amount, currentRound, bettingPhase) {
...
// ─── Main command ─────────────────────────────────────────────────────────
export async function pokerRoundCommand(gameId, handValue, opts) {
    ...
    let game = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "getGame");
    if (game.settled) {
        ok({
            gameId: Number(gid),
            gameComplete: true,
    ...
    if (Number(game.phase) === PHASE_COMMIT) {
        ...
        while (true) {
            ...
            game = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-commit");
            if (game.settled) {
    ...
    if (Number(game.phase) === POKER_BETTING1) {
        ...
        while (true) {
            ...
            game = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-betting1");
            if (game.settled) {
    ...
    if (Number(game.phase) === POKER_BETTING2) {
        ...
        while (true) {
            ...
            game = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-betting2");
            if (game.settled) {
                ok({ gameId: Number(gid), gameComplete: true, p1Score: Number(game.p1Score), p2Score: Number(game.p2Score), message: "Game settled (fold or timeout)." });
                return;
            }
    ...
    if (Number(game.phase) === POKER_SHOWDOWN) {
        ...
        if (!myRevealed) {
            ...
            for (let revealAttempt = 0; revealAttempt < 3; revealAttempt++) {
                try {
                    ...
                    const gCheck = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "getGame-after-revert");
        ...
        while (true) {
            ...
            game = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-showdown");
...
function returnRoundResult(game, gid, playedRound, totalRounds, isPlayer1) {
    const myScore = isPlayer1 ? Number(game.p1Score) : Number(game.p2Score);
    const oppScore = isPlayer1 ? Number(game.p2Score) : Number(game.p1Score);
    const myBudget = isPlayer1 ? Number(game.p1Budget) : Number(game.p2Budget);
...
//# sourceMappingURL=poker-round.js.map