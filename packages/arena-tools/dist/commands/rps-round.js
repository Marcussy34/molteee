...
const MOVE_MAP = { rock: 1, paper: 2, scissors: 3 };
const MOVE_NAMES = { 1: "Rock", 2: "Paper", 3: "Scissors" };
// Phase enums
...
function roundWinner(p1Move, p2Move) {
    if (p1Move === p2Move)
        return "draw";
...
// ─── Main command ─────────────────────────────────────────────────────────
export async function rpsRoundCommand(gameId, move) {
    ...
    // Read game state
    const game = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "getGame");
    if (game.settled) {
        ok({
            gameId: Number(gid),
            gameComplete: true,
    ...
    // ── Step 1: Commit ──────────────────────────────────────────────────────
    const roundData = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound");
    ...
    while (true) {
        ...
        const g = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "poll-commit");
        if (g.settled) {
    ...
    const roundData2 = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound-reveal");
    ...
    while (true) {
        ...
        const g = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getGame", args: [gid] }), "poll-reveal");
        // Game settled — final result
        if (g.settled) {
            // Read final round data for move info
            const finalRound = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound-final");
        ...
        if (Number(g.currentRound) > round) {
            const finalRound = await retry(() => client.readContract({ address: addr, abi: rpsGameAbi, functionName: "getRound", args: [gid, BigInt(round)] }), "getRound-result");
            const p1Move = Number(finalRound.p1Move);
            const p2Move = Number(finalRound.p2Move);
            const myMove = isPlayer1 ? p1Move : p2Move;
            const oppMove = isPlayer1 ? p2Move : p1Move;
...
//# sourceMappingURL=rps-round.js.map