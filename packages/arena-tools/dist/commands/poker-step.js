...
export async function pokerStepCommand(gameId, decision, opts) {
    ...
    if (phase === PHASE_COMMIT) {
        ...
        while (true) {
            ...
            const g = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-commit");
            if (g.settled) {
    ...
    if (phase === POKER_BETTING1 || phase === POKER_BETTING2) {
        ...
        while (true) {
            ...
            const g = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-betting");
            if (g.settled) {
                ok({
                    gameId: Number(gid),
                    gameComplete: true,
    ...
    if (phase === POKER_SHOWDOWN) {
        ...
        while (true) {
            ...
            const g = await retry(() => client.readContract({ address: addr, abi: pokerGameV2Abi, functionName: "getGame", args: [gid] }), "poll-showdown");
            if (g.settled) {
