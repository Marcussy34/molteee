...
// ─── Main command ─────────────────────────────────────────────────────────
export async function pokerStepCommand(gameId, decision, opts) {
    const client = getPublicClient();
    const myAddress = getAddress();
    const gid = BigInt(gameId);
    const addr = CONTRACTS.PokerGame;
    // Read game state
    const game = await retry(() => client.readContract({ address: addr, abi: pokerGameAbi, functionName: "getGame", args: [gid] }), "getGame");
    if (game.settled) {
        ok({
            gameId: Number(gid),
            gameComplete: true,
    ...
    if (phase === PHASE_COMMIT) {
        ...
        while (true) {
            ...
            const g = await retry(() => client.readContract({ address: addr, abi: pokerGameAbi, functionName: "getGame", args: [gid] }), "poll-commit");
            if (g.settled) {
    ...
    if (phase === POKER_BETTING1 || phase === POKER_BETTING2) {
        ...
        while (true) {
            ...
            const g = await retry(() => client.readContract({ address: addr, abi: pokerGameAbi, functionName: "getGame", args: [gid] }), "poll-betting");
            if (g.settled) {
    ...
    if (phase === POKER_SHOWDOWN) {
        ...
        while (true) {
            ...
            const g = await retry(() => client.readContract({ address: addr, abi: pokerGameAbi, functionName: "getGame", args: [gid] }), "poll-showdown");
            if (g.settled) {
...
//# sourceMappingURL=poker-step.js.map