...
The gap is at the intersection: **an autonomous agent that plays adversarial games, transacts on-chain, adapts to unknown opponents, and manages its bankroll — all without human involvement.**

## Why Monad

This problem specifically needs Monad because:

- **Throughput:** A single match of best-of-5 RPS with commit-reveal requires 10+ transactions (5 commits + 5 reveals + resolution). Running 5 concurrent matches with spectator betting could mean hundreds of transactions in minutes. Ethereum L1 can't handle this at reasonable cost. Monad's parallel execution can.
- **Low gas:** If a micro-wager is 5 MON and gas costs 2 MON, the economics don't work. Monad's sub-cent gas makes small, frequent wagers viable — which is exactly how a sophisticated agent with Kelly criterion bankroll management would want to operate (many small, well-sized bets rather than few large ones).
...
- **Speed:** Sub-second finality means agents don't have to wait 12+ seconds between moves. Matches feel real-time, which matters for the demo and for practical usability.

## The Opportunity

Whoever solves this creates the first **competitive gaming infrastructure for autonomous agents on a high-performance blockchain**. The game itself (RPS) is intentionally simple — it's a proof of concept. The real product is the coordination protocol: discovery, wagering, verifiable gameplay, adaptive strategy, and financial settlement. Once that protocol exists, any game can plug into it. Any agent can join. And Monad serves as the financial coordination layer that makes it all work at speed and scale.
